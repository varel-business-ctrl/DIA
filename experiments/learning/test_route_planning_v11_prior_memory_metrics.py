"""DIA V11 - PRIOR MEMORY CAUSAL METRICS
...
def execute(s: Scenario, mode: Mode, memory: Optional[KnowledgeMemory], max_actions: int = 100) -> Result:
    ...
    prior_memory = set(local_known)
    remembered_available = len(prior_memory)
    ...
    while actions < max_actions:
        ...
        if mode != Mode.NO_MEMORY and prior_memory:
            unconstrained = model.bfs(position, s.target, set())
            constrained = model.bfs(position, s.target, local_known)
            if unconstrained is not None and constrained is not None:
                excluded_now = set(unconstrained[1:]) - set(constrained[1:])
                remembered_route_excluded += len(excluded_now & prior_memory)
...
def print_report(all_results: Dict[str, List[Result]]) -> None:
    print("DIA V11 - PRIOR MEMORY CAUSAL METRICS")
    print("Memory is learned only from failed movements.")
    print("Only knowledge present before the current trial counts as prior memory.")
    print("Memory influence is measured by comparing unconstrained and memory-constrained routes.\n")
